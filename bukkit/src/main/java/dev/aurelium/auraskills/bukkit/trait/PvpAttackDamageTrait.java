...
public class PvpAttackDamageTrait extends TraitImpl {

    PvpAttackDamageTrait(AuraSkills plugin) {
        super(plugin, Traits.PVP_ATTACK_DAMAGE);
    }
    ...
    public void damageListener(DamageEvent event) {
        ...
        if (player != null & target != null) {
            ...
            Trait trait = Traits.PVP_ATTACK_DAMAGE;
    ...
    private DamageModifier applyStrength(User user) {
        double value = user.getBonusTraitLevel(Traits.PVP_ATTACK_DAMAGE);
        if (Traits.PVP_ATTACK_DAMAGE.optionBoolean("use_percent")) {
