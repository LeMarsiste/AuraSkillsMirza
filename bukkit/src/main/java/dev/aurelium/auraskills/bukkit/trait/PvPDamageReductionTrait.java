...
public class PvPDamageReductionTrait extends TraitImpl {

    @Nullable
    private Expression formula;

    PvPDamageReductionTrait(AuraSkills plugin) {
        super(plugin, Traits.PVP_DAMAGE_REDUCTION);
    }
    ...
    public void damageListener(DamageEvent event) {
        if (!Traits.PVP_DAMAGE_REDUCTION.isEnabled()) {
            return;
        }
        ...
        if (player != null & target != null) {
            ...
            double reduction = user.getEffectiveTraitLevel(Traits.PVP_DAMAGE_REDUCTION);
    ...
    private double getReductionValue(double value) {
        Trait trait = Traits.PVP_DAMAGE_REDUCTION;
        try {
            if (formula == null) {
